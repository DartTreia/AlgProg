...
public int findId(String name){
   ...
   public Vertex[] stepsLoopVert(String name, Vertex[] visVerts){
    ...
    public Vertex[] BFSVertsLibQueue(String name){
        Vertex tmp = this.next;
        while(tmp!=null){
            if(tmp.vertex.equals(name)) break;
            tmp=tmp.next;
        ...
        while(!libQueue.isEmpty()){
            ...
            if(newVert.adjVertexes!=null) {
                for (int i = 0; i < newVert.adjVertexes.length; i++) {
                    final Vertex vertex = newVert.adjVertexes[i];
            ...
            if(newVert.adjVertexes!=null) {
                for (int i = 0; i < newVert.adjVertexes.length; i++) {
                    ...
                    if (Arrays.stream(visVerts).noneMatch(n -> n == vertex)) {
                        ...
                        tempVerts[visVerts.length] = vertex;
                        visVerts = tempVerts;
                        dists[Integer.parseInt(vertex.vertex.substring(1))-1]=dists[Integer.parseInt(newVert.vertex.substring(1))-1]+1;
                    }
                }
            }
        }
